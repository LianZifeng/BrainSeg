...
def random_transform_centered(tumor_data):
    ...
    if any(150 > dim > 100 for dim in tumor_data.shape):
        ...
        new_tumor_data[center[0] - shape[0] // 2 : center[0] - shape[0] // 2 + shape[0], center[1] - shape[1] // 2 : center[1] - shape[1] // 2 + shape[1], center[2] - shape[2] // 2 : center[2] - shape[2] // 2 + shape[2]] = tumor_data
...
def gaussian_kernel_3d(ksize=5, sigma=1.2, device='cuda'):
    ax = torch.arange(-ksize // 2 + 1., ksize // 2 + 1., device=device)
...
def masked_gaussian_blur_3d(img, mask, ksize=5, sigma=1.2):
    """
    img:  [D, H, W]  torch Tensor
    mask: [D, H, W]  bool Tensor
    """
    device = img.device
    kernel = gaussian_kernel_3d(ksize, sigma, device)
    kernel = kernel.view(1, 1, ksize, ksize, ksize)

    img_in = img.unsqueeze(0).unsqueeze(0)
    blur = F.conv3d(img_in, kernel, padding=ksize // 2)[0, 0]
...
def real_DWI_flair_lesion(brain_data, roi_data, tissue_data, meta_tumor, lesion_path, modality_key, example_path=None, downsample=False):
    ...
    tumor_data = sitk.GetArrayFromImage(sitk.ReadImage(example_path))
    ...
    for ii in index_all:
        if int(ii)>=107:
            mu = tumor_syn_parameter[(int(ii) - 106) * 2] + 60
            ...
            mask = (label == int(ii))
            gen_ima = masked_gaussian_blur_3d(
                gen_ima[0,0], mask[0,0],
                ksize=5,
                sigma=1.2
