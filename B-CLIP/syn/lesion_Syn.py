...
def find_bounding_box(image):
    ...
    bbox_coordinates = [(bbox[0].start, bbox[0].stop, bbox[1].start, bbox[1].stop, bbox[2].start, bbox[2].stop) for bbox
                        in bbox_slices]
...
if '__main__' == __name__:
    import nibabel as nib
    import pandas as pd
    affine = nib.load(r'C:\Users\Zifeng Lian\Desktop\UKBiobank2_3593230\DWI-brain.nii.gz').affine
    DWI = nib.load(r'C:\Users\Zifeng Lian\Desktop\UKBiobank2_3593230\DWI-brain.nii.gz').get_fdata()
    tissue = nib.load(r'C:\Users\Zifeng Lian\Desktop\UKBiobank2_3593230\tissue.nii.gz').get_fdata()
    dk_struct = nib.load(r'C:\Users\Zifeng Lian\Desktop\UKBiobank2_3593230\dk-struct.nii.gz').get_fdata()
    lesion_path = r'E:\stroke'
    meta_table_lesion = pd.read_excel(r'C:\Users\Zifeng Lian\Desktop\lesion_parameter.xlsx')
    DWI, dk_struct, tissue = real_DWI_flair_lesion(DWI, dk_struct, tissue, 0, 0, 0, 0, meta_table_lesion, lesion_path, 'DWI-brain.nii.gz')
    DWI = DWI.cpu().numpy()
    tissue = tissue.squeeze().cpu().numpy()
    dk_struct = dk_struct.squeeze().cpu().numpy()
    a = nib.Nifti1Image(DWI, affine=affine)
    b = nib.Nifti1Image(tissue, affine=affine)
    c = nib.Nifti1Image(dk_struct, affine=affine)
    nib.save(a, r'C:\Users\Zifeng Lian\Desktop\1\DWI-brain.nii.gz')
    nib.save(b, r'C:\Users\Zifeng Lian\Desktop\1\tissue.nii.gz')
    nib.save(c, r'C:\Users\Zifeng Lian\Desktop\1\dk-struct.nii.gz')